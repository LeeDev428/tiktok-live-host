...
try {
    ...
    $all_sellers = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    die("Failed to fetch sellers: " . $e->getMessage());
}

// Build the main query - Simplified to avoid collation issues
$query = "
    SELECT 
        a.id,
        a.attendance_date,
    ...
    AND LENGTH(a.total_sold_photo) > 0
";
...
if (!empty($selected_date)) {
    ...
    // Extract year and month parts to avoid string comparison
    ...
    if (count($year_month) == 2) {
        ...
        $params[':year'] = (int)$year_month[0];
        $params[':month'] = (int)$year_month[1];
    }
}
...
try {
    ...
    die("Query failed: " . $e->getMessage() . "<br><br>Query: " . htmlspecialchars($query));
}

// Now fetch user and time slot data separately (avoids collation JOIN issues)
$attendance_records = [];
foreach ($attendance_data as $record) {
    // Fetch user data
    try {
        $user_stmt = $db->prepare("SELECT id, full_name, profile_image, experienced_status FROM users WHERE id = ?");
        $user_stmt->execute([$record['seller_id']]);
        $user = $user_stmt->fetch(PDO::FETCH_ASSOC);
        
        if ($user) {
            $record['user_id'] = $user['id'];
            $record['full_name'] = $user['full_name'];
            $record['profile_image'] = $user['profile_image'];
            $record['experienced_status'] = $user['experienced_status'];
        } else {
            $record['user_id'] = $record['seller_id'];
            $record['full_name'] = 'Unknown User';
            $record['profile_image'] = null;
            $record['experienced_status'] = 'newbie';
        }
    } catch (PDOException $e) {
    ...
    // Fetch time slot data if exists
    if (!empty($record['time_slot'])) {
        try {
            $slot_stmt = $db->prepare("SELECT name, start_time, end_time, duration_hours FROM attendance_time_slots WHERE id = ?");
            $slot_stmt->execute([(int)$record['time_slot']]);
            ...
            if ($slot) {
                ...
                $record['duration_hours'] = $slot['duration_hours'];
            } else {
                $record['slot_name'] = null;
                $record['start_time'] = null;
                $record['end_time'] = null;
                $record['duration_hours'] = null;
            }
        } catch (PDOException $e) {
            // If slot fetch fails, set to null
            $record['slot_name'] = null;
            $record['start_time'] = null;
            $record['end_time'] = null;
            $record['duration_hours'] = null;
...
// Get statistics - Simplified to avoid collation issues
$stats_query = "
    SELECT 
...
if ($selected_user !== 'all') {
    ...
    $stats_params[':user_id'] = (int)$selected_user;
}
if (!empty($selected_date)) {
    $stats_query .= " AND a.attendance_date = :date";
    $stats_params[':date'] = $selected_date;
} elseif (!empty($selected_month)) {
    // Use YEAR/MONTH functions instead of DATE_FORMAT to avoid collation
    $year_month = explode('-', $selected_month);
    if (count($year_month) == 2) {
        $stats_query .= " AND YEAR(a.attendance_date) = :year AND MONTH(a.attendance_date) = :month";
        $stats_params[':year'] = (int)$year_month[0];
        $stats_params[':month'] = (int)$year_month[1];
    }
}
    ...
    <div class="filters-section">
        <form method="GET" action="">
            ...
            <div class="filter-actions">
                ...
                <button type="button" class="btn-reset" onclick="window.location.href='attendance-photos.php'">🔄 Reset</button>
            </div>
        </form>
    </div>
        ...
        <div class="photos-grid">
            <?php foreach ($attendance_records as $record): ?>
                <div class="photo-card">
                    ...
                    <?php
                    // Handle image path - check if it already has uploads/ prefix
                    $photo_filename = $record['total_sold_photo'];
                    if (strpos($photo_filename, 'uploads/') === 0) {
                        // Already has full path from root
                        $image_src = '../' . $photo_filename;
                    } else {
                        // Just filename, add full path
                        $image_src = '../uploads/attendance/' . $photo_filename;
                    }
                    ?>
                    
                    <div class="photo-image-container" onclick="openPhotoModal('<?php echo htmlspecialchars($image_src); ?>')">
